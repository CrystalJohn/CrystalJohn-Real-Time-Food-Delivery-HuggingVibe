...
Frontend được tổ chức theo tinh thần giống backend: **feature-first (Option A)** + **BCE mindset**.

## 1. Routing theo Actor (App Router)

Routes được nhóm theo actor để traceability theo flows rõ ràng:

- `(customer)` : Menu, Cart, Checkout, Order Tracking
- `(staff)`    : Queue PENDING, Accept/Reject/Preparing/Ready
- `(driver)`   : Job list (READY), Delivery detail + gửi location realtime
- `(admin)`    : Dashboard, Driver approval, Statistics

## 2. Feature Modules (Option A)

Business logic UI được tổ chức theo module tương tự backend:
...
Mỗi module có các phần:
- `boundary/` : UI components/forms (hiển thị + input)
- `control/`  : client use-cases (gọi API, điều phối state)
- `entity/`   : TS models/types (pure types)
- `ports/`    : interfaces cho data/realtime access

## 3. Infrastructure layer

`src/infrastructure/` chứa adapters kỹ thuật dùng chung:
- `api/`      : fetch wrapper, base endpoints, token storage (simple)
- `realtime/` : websocket client, channels/events

> **Rule:** Page/Component không gọi `fetch()` hay WS trực tiếp. Phải gọi qua `control/` (use cases) của module.
